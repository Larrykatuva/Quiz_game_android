...
public class Question9Fragment extends Fragment implements View.OnClickListener{
    ...
    int remaining = 885000;
    private TextView Tquestion, A, B, C, D;
    private Button btnA, btnB, btnC, btnD;
    int saved;

    public Question9Fragment(ArrayList<Question> questions, int saved){
    ...
    public void onClick(View v) {
        switch (v.getId()){
            case R.id.btnB:
                getFragmentManager()
                        .beginTransaction()
                        .replace(R.id.questions_holder, new Question10Fragment(questions,saved))
                        .commit();
                break;
            case R.id.btnA:
            case R.id.btnC:
