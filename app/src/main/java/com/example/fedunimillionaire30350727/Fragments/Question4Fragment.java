...
public class Question4Fragment extends Fragment implements View.OnClickListener{
    ...
    int remaining = 996000;
    private TextView Tquestion, A, B, C, D;
    private Button btnA, btnB, btnC, btnD;
    int saved;

    public Question4Fragment(ArrayList<Question> questions, int saved){
    ...
    public void onClick(View v) {
        switch (v.getId()){
            case R.id.btnA:
                getFragmentManager()
                        .beginTransaction()
                        .replace(R.id.questions_holder, new Question5Fragment(questions,saved))
                        .commit();
                break;
            case R.id.btnB:
            case R.id.btnC:
