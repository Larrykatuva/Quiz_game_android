...
public class Question2Fragment extends Fragment implements View.OnClickListener{
    ...
    int remaining = 999000;
    private TextView Tquestion, A, B, C, D;
    private Button btnA, btnB, btnC, btnD;
    int saved;

    public Question2Fragment(ArrayList<Question> questions, int saved){
    ...
    public void onClick(View v) {
        switch (v.getId()){
            case R.id.btnC:
                getFragmentManager()
                        .beginTransaction()
                        .replace(R.id.questions_holder, new Question3Fragment(questions,saved))
                        .commit();
                break;
            case R.id.btnA:
                getFragmentManager()
