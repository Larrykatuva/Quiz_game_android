...
public class Question5Fragment extends Fragment implements View.OnClickListener{
    ...
    int remaining = 992000;
    private TextView Tquestion, A, B, C, D;
    private Button btnA, btnB, btnC, btnD;
    int saved;
    public Question5Fragment(ArrayList<Question> questions, int saved){
    ...
    public void onClick(View v) {
        switch (v.getId()){
            case R.id.btnD:
                getFragmentManager()
                        .beginTransaction()
                        .replace(R.id.questions_holder, new Question6Fragment(questions,saved))
                        .commit();
                break;
            case R.id.btnA:
            case R.id.btnC:
